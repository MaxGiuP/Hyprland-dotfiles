...
Item {
    ...
    readonly property list<PwNode> appPwNodes: Pipewire.nodes.values.filter((node) => {
        ...
        // const mc = String(n.mediaClass || "").toLowerCase()
        // Typical values: "stream/output/audio", "stream/input/audio"
        // return mc.includes("stream") && mc.includes("audio") && mc.includes("hda") && mc.includes("combined")
    })
