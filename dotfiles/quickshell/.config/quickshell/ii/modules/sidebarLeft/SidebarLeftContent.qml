...
Item {
    ...
    property var tabButtonList: [
        ...
        ...(Config.options.policies.weeb === 1 ? [{"icon": "bookmark_heart", "name": Translation.tr("Images")}] : []),
        {"icon": "terminal", "name": Translation.tr("Console")}
    ]
    property int selectedTab: 0
    property int tabs: root.tabButtonList.length
    ...
    Keys.onPressed: (event) => {
        if (event.modifiers === Qt.ControlModifier) {
            if (event.key === Qt.Key_PageDown) {
                root.selectedTab = Math.min(root.selectedTab + 1, tabs - 1)
                event.accepted = true;
            } 
                ...
                root.selectedTab = (root.selectedTab + 1) % tabs;
                event.accepted = true;
            }
            else if (event.key === Qt.Key_Backtab) {
                root.selectedTab = (root.selectedTab - 1 + tabs) % tabs;
                event.accepted = true;
            }
        }
    }
