...
Item {
    ...
    const pinnedLower = pinnedApps.map(a => a.toLowerCase());

    // ➜ Funzione di normalizzazione: se esiste un pin "org.mozilla.thunderbird"
    // e arriva una finestra "thunderbird", usa sempre l'ID pinned.
function canonicalId(id) {
    const l = (id || "").toLowerCase();
    const aliases = {
        "thunderbird": "org.mozilla.thunderbird",
        "vmware": "vmware-workstation"  // Workstation Pro usa WM_CLASS=vmware
    };

    if (pinnedLower.includes(l)) return l;
    if (aliases[l]) {
        const target = aliases[l];
        if (pinnedLower.includes(target)) return target;
        return target;
    }
    for (const p of pinnedLower) {
        if (p.endsWith("." + l)) return p;
        if (l.endsWith("." + p)) return l;
    }
    return l;
}
    ...
    for (const appId of pinnedApps) {
        const key = canonicalId(appId);
        if (!map.has(key)) map.set(key, ({ pinned: true, toplevels: [] }));
    }
    ...
    // 3) Finestra aperte ➜ usa canonicalId QUI
    for (const toplevel of ToplevelManager.toplevels.values) {
        if (ignoredRegexes.some(re => re.test(toplevel.appId))) continue;
        const key = canonicalId(toplevel.appId);   // <— QUESTA È LA RIGA CHE FA LA MAGIA
        if (!map.has(key)) map.set(key, ({ pinned: false, toplevels: [] }));
