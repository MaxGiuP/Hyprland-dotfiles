...
Scope {
    ...
    component DescriptionLabel: Rectangle {
        ...
        implicitHeight: descriptionLabelText.implicitHeight + 20
        implicitWidth: descriptionLabelText.implicitWidth + 30
    ...
    function closeAllWindows() {
        HyprlandData.windowList.map(w => w.pid).forEach(pid => Quickshell.execDetached(["kill", pid]));
    }
    ...
    Process {
        ...
        onExited: (code) => root.packageManagerRunning = (code === 0)
    }

    Process {
        id: detectDownloadProc
        command: ["bash","-c","pidof curl wget aria2c yt-dlp || ls ~/Downloads | grep -E '\\.crdownload$|\\.part$'"]
        onExited: (code) => root.downloadRunning = (code === 0)
    }

    Loader {
        id: sessionLoader
        active: GlobalStates.sessionOpen
        onActiveChanged: if (active) root.detectRunningStuff()

        Connections {
            target: GlobalStates
            function onScreenLockedChanged() { if (GlobalStates.screenLocked) GlobalStates.sessionOpen = false }
        }
        ...
        sourceComponent: PanelWindow {
            ...
            // Nav state
            property bool hasKeyboardSelection: false
            property var firstButton: sessionLock
            ...
            function allButtons() {
                return [
                    sessionLock, sessionSleep, sessionLogout, sessionTaskManager,
                    sessionHibernate, sessionShutdown, sessionReboot, sessionFirmwareReboot
                ];
            }

            function clearAllFocus() {
                for (let b of allButtons()) if (b) b.focus = false;
            }

            // Called by buttons on hover enter
            function mouseTakeFocus(btn) {
                hasKeyboardSelection = false;
                clearAllFocus();
                if (btn) Qt.callLater(() => btn.forceActiveFocus());
            }

            // Called by buttons on hover leave
            function mouseReleaseFocus() {
                clearAllFocus();
                Qt.callLater(() => contentColumn.forceActiveFocus());
            }
            ...
            onVisibleChanged: if (visible) resetOnOpen()

            MouseArea { anchors.fill: parent; onClicked: sessionRoot.hide() }
            ...
            ColumnLayout {
                ...
                Keys.onPressed: (event) => {
                    if (event.key === Qt.Key_Escape) { sessionRoot.hide(); return }
                    if (!sessionRoot.hasKeyboardSelection &&
                        (event.key === Qt.Key_Left || event.key === Qt.Key_Right ||
                         event.key === Qt.Key_Up   || event.key === Qt.Key_Down)) {
                        sessionRoot.hasKeyboardSelection = true;
                        sessionRoot.firstButton.forceActiveFocus();
                        event.accepted = true;
                        return;
                    }
                }
            ...
            RowLayout {
                anchors { top: contentColumn.bottom; topMargin: 10; horizontalCenter: contentColumn.horizontalCenter }
                spacing: 10
                Loader {
