...
page=$((1 + RANDOM % 1000));
response=$(curl "https://wallhaven.cc/api/v1/search?categories=100&purity=100&sorting=random&page=$page&q=landscape")
link=$(echo "$response" | jq -r '.data[0].path')
ext=$(echo "$link" | awk -F. '{print $NF}')
downloadPath="$PICTURES_DIR/Wallpapers/random_image.$ext"
illogicalImpulseConfigPath="$HOME/.config/illogical-impulse/config.json"
currentWallpaperPath=$(jq -r '.background.wallpaperPath' $illogicalImpulseConfigPath)
if [ "$downloadPath" == "$currentWallpaperPath" ]; then
    downloadPath="$PICTURES_DIR/Wallpapers/random_image-1.$ext"
fi
