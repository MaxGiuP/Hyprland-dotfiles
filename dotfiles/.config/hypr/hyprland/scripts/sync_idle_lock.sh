...
lock_status_cmd = "~/.config/hypr/hyprland/scripts/quickshell_session_is_locked.sh ii"
block = (
    f"{start}\n"
    ...
    "# a stale idle inhibitor. The condition prevents bypassing coffee mode.\n"
    "listener {\n"
    ...
    "    ignore_inhibit = true\n"
    f"    condition_cmd = {lock_status_cmd}\n"
    "    condition_retry = 15\n"
    "}\n\n"
    "# Suspend only after a real Wayland session lock is active. Hypridle's\n"
    "# inhibit_sleep=3 also holds external sleep requests until that lock lands.\n"
    "listener {\n"
