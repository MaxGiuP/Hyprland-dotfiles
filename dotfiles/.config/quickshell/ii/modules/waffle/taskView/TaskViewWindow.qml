...
WMouseAreaButton {
    ...
    function closeWindow() {
        HyprlandDispatch.dispatch(`closewindow address:${root.hyprlandClient?.address}`);
    }
    ...
    onClicked: event => {
        if (event.button === Qt.LeftButton) {
            ...
            HyprlandDispatch.dispatch(`focuswindow address:${root.hyprlandClient?.address}`);
            GlobalStates.overviewOpen = false;
    ...
    WMenu {
        ...
        Action {
            ...
            onTriggered: {
                HyprlandDispatch.dispatch(`pin address:${root.hyprlandClient?.address}`);
            }
        }
        Action {
            icon.name: "empty"
