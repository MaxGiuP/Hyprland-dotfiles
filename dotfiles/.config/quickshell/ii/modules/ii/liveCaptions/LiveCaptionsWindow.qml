...
Scope {
    id: root
    readonly property string targetScreenName: GlobalStates.resolvedLiveCaptionsScreen()
    readonly property ShellScreen targetShellScreen: Quickshell.screens.find(s => s.name === targetScreenName) ?? Quickshell.screens[0] ?? null
    readonly property real topOffset: (GlobalStates.barTopClearanceByScreen[targetScreenName] ?? Appearance.sizes.barHeight) + 12
    ...
    Loader {
        active: GlobalStates.liveCaptionsOpen
        sourceComponent: PanelWindow {
            id: liveCaptionsWindow
            visible: true
            screen: root.targetShellScreen
            exclusionMode: ExclusionMode.Ignore
            exclusiveZone: 0
            color: "transparent"

            WlrLayershell.namespace: "quickshell:liveCaptions"
            WlrLayershell.layer: WlrLayer.Top
            WlrLayershell.keyboardFocus: WlrKeyboardFocus.OnDemand

            anchors { top: true; left: true; right: true }
            margins { top: root.topOffset; left: 0; right: 0; bottom: 0 }

            implicitWidth: screen?.width ?? 1000
            implicitHeight: captionCard.implicitHeight + Appearance.sizes.elevationMargin * 2

            mask: Region { item: captionCardWrap }

            Item {
                id: captionCardWrap
                anchors.top: parent.top
                anchors.horizontalCenter: parent.horizontalCenter
                implicitWidth: Math.min((liveCaptionsWindow.screen?.width ?? 1000) - 48, 680)
                implicitHeight: captionCard.implicitHeight + Appearance.sizes.elevationMargin * 2

                StyledRectangularShadow { target: captionCard }

                Rectangle {
                    id: captionCard
                    anchors.centerIn: parent
                    width: captionCardWrap.implicitWidth - Appearance.sizes.elevationMargin
                    implicitHeight: captionLayout.implicitHeight + 22
                    ...
                    ColumnLayout {
                        ...
                        RowLayout {
                            ...
                            DialogButton {
                                ...
                                downAction: () => GlobalStates.closeLiveCaptions()
                            }
                        }
                        ...
                        // Translate to (only when translation box is enabled)
                        StyledText {
                            Layout.fillWidth: true
