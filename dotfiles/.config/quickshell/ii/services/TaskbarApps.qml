...
Singleton {
    ...
    function movePinnedApp(appId, targetAppId = "") {
        ...
        const reorderedApps = pinnedApps.filter(id => id !== appId);
        let targetIndex = targetAppId.length === 0 ? reorderedApps.length : reorderedApps.indexOf(targetAppId);
        if (targetIndex < 0) targetIndex = reorderedApps.length;
        reorderedApps.splice(targetIndex, 0, appId);
