...
Singleton {
    ...
    property var models: {
        "ollama-default": aiModelComponent.createObject(this, {
            "name": "Ollama Default (llama3.2:3b)",
            "icon": "linux-symbolic",
            "description": Translation.tr("Local fallback model"),
        ...
        "gemini-3-flash-preview": aiModelComponent.createObject(this, {
            ...
            "description": Translation.tr("Online | Google's latest frontier Flash family preview model."),
            ...
            "key_get_description": Translation.tr("Get a Gemini API key in AI Studio."),
            "api_format": "gemini",
        }),
        "gemini-3-pro-preview": aiModelComponent.createObject(this, {
            "name": "Gemini 3 Pro Preview",
            "icon": "google-gemini-symbolic",
            "description": Translation.tr("Online | Google's latest Pro preview for advanced reasoning and coding."),
            "homepage": "https://aistudio.google.com",
        ...
        "gemini-flash-latest": aiModelComponent.createObject(this, {
            ...
            "icon": "google-gemini-symbolic",
            "description": Translation.tr("Online | Alias to Google's latest Flash release."),
            "homepage": "https://aistudio.google.com",
    ...
    readonly property var previewModelList: root.modelList.filter(modelId => {
        const model = root.models[modelId];
        if (!model) return false;
        const isLocal = (model.endpoint ?? "").includes("localhost");
        return isLocal ? root.ollamaRunning : true;
    })
    property var currentModelId: Persistent.states?.ai?.model || modelList[0]
    property list<string> dynamicOnlineModelIds: []
    ...
    function guessModelLogo(model) {
        if (model.includes("llama")) return "linux-symbolic";
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: data => {
                try {
                    ...
                    const registered = [];
                    root.localOllamaModels = dataJson;
                    root.ollamaInstalled = true;
                    dataJson.forEach(model => {
                        const safeModelName = root.safeModelName(model);
                        registered.push(safeModelName);
        ...
        onExited: (exitCode, exitStatus) => {
            if (exitCode !== 0) {
                root.ollamaInstalled = false;
                root.localOllamaModels = [];
                root.removeModels(root.dynamicOllamaModelIds);
    ...
    Process {
        ...
        environment: ({
            "GEMINI_API_KEY": root.apiKeys?.gemini ?? "",
            "OPENROUTER_API_KEY": root.apiKeys?.openrouter ?? "",
            "MISTRAL_API_KEY": root.apiKeys?.mistral ?? "",
        })
        stdout: StdioCollector {
            id: onlineModelsCollector
            onStreamFinished: {
                if (!onlineModelsCollector.text || onlineModelsCollector.text.trim().length === 0) return;
                try {
                    const payload = JSON.parse(onlineModelsCollector.text.trim());
                    ...
                    discovered.forEach(model => {
                        ...
                        root.addModel(modelKey, {
                            ...
                            "icon": model.icon ?? "language-symbolic",
                            "description": `Online | ${provider} | ${modelCode}`,
                            "homepage": model.homepage ?? "",
                            "endpoint": model.endpoint,
                            "model": modelCode,
                            "requires_key": model.requires_key ?? true,
                            "key_id": model.key_id ?? provider,
                            "key_get_link": provider === "gemini" ? "https://aistudio.google.com/app/apikey" :
                                (provider === "mistral" ? "https://console.mistral.ai/api-keys" : "https://openrouter.ai/settings/keys"),
                            "key_get_description": model.key_id === "gemini"
                                ? Translation.tr("Get a Gemini API key in AI Studio.")
                                : (model.key_id === "mistral"
    ...
    function setModel(modelId, feedback = true, setPersistentState = true) {
        ...
        if (modelList.indexOf(modelId) !== -1) {
            ...
            if ((model.endpoint ?? "").includes("localhost") && !root.ollamaRunning) {
                root.addMessage(Translation.tr("Ollama is down. Start Ollama before selecting local models."), root.interfaceRole);
                return;
            }
            if (setPersistentState) Persistent.states.ai.model = modelId;
            root.currentModelId = modelId;
            root.currentApiStrategy = apiStrategies[model.api_format || "openai"];
            ...
            if (root.modelList.length > 0) {
                const fallbackId = root.modelList[0];
                if (setPersistentState) Persistent.states.ai.model = fallbackId;
                root.currentModelId = fallbackId;
                root.currentApiStrategy = apiStrategies[root.models[fallbackId]?.api_format || "openai"];
            }
